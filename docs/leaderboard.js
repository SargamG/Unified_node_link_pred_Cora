...
function renderTable(){
  ...
  rows.forEach((r, idx) => {
    ...
    cells.forEach(([k, v]) => {
      ...
      if(k === "node_f1") td.classList.add("node_f1");
      if(k === "link_auc") td.classList.add("link_auc");
...
function applyFilters(){
  ...
  rows.sort((a,b) => {
    let av = a[k], bv = b[k];
    if(k === "score" || k === "node_f1" || k === "link_auc"){
  ...
  document.querySelectorAll("#tbl thead th").forEach(th => {
    th.addEventListener("click", () => {
      ...
      if(state.sortKey === k){
        ...
        state.sortKey = k;
        state.sortDir = (k === "score" || k === "node_f1" || k === "link_auc") ? "desc" : "asc";
      }
      applyFilters();
    });
  });
